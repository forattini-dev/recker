...
export class YouTubeExtractor extends BaseExtractor {
  ...
  async extract(url: string): Promise<ExtractorResult> {
    ...
    const { videoDetails, streamingData, playabilityStatus, captions } = playerResponse;
    ...
    // Extract subtitles
    const { subtitles, automaticCaptions } = this.extractSubtitles(captions);
    ...
    return {
      ...
      automaticCaptions: Object.keys(automaticCaptions).length > 0 ? automaticCaptions : undefined,
    };
  }
  ...
  /**
   * Extract subtitles from captions data
   */
  private extractSubtitles(captions: YouTubePlayerResponse['captions']): {
    subtitles: Subtitles;
    automaticCaptions: Subtitles;
  } {
    const subtitles: Subtitles = {};
    const automaticCaptions: Subtitles = {};
    ...
    for (const track of captionTracks) {
      ...
      const name = track.name?.simpleText || track.name?.runs?.map(r => r.text).join('') || langCode;

      // Generate subtitle tracks for different formats
      const formats: Array<{ ext: SubtitleTrack['ext']; fmt: string }> = [
        { ext: 'vtt', fmt: 'vtt' },
        { ext: 'srv3', fmt: 'srv3' },
        { ext: 'ttml', fmt: 'ttml' },
        { ext: 'json3', fmt: 'json3' },
      ...
      const tracks: SubtitleTrack[] = formats.map(({ ext, fmt }) => ({
        url: `${track.baseUrl}&fmt=${fmt}`,
        ext,
        name,
        isAutoGenerated,
      }));

      // Separate manual from auto-generated
      if (isAutoGenerated) {
        if (!automaticCaptions[langCode]) {
          automaticCaptions[langCode] = [];
        }
        automaticCaptions[langCode].push(...tracks);
      } else {
        if (!subtitles[langCode]) {
          subtitles[langCode] = [];
        }
        subtitles[langCode].push(...tracks);
      }
    }
