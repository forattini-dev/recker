...
export default class HTMLElement extends Node {
  ...
  public set innerHTML(content: string) {
    ...
    const nodes = r.childNodes.length
      ? r.childNodes
      : [new TextNode(content, this as any)];
  ...
  public set_content(
    ...
    if (content instanceof Node) {
      ...
      content = r.childNodes.length
        ? r.childNodes
        : [new TextNode(r.innerHTML, this as any)];
    }
    resetParent(this.childNodes, null);
  ...
  public replaceWith(...nodes: (string | Node)[]) {
    ...
    if (!parent) return this; // Cannot replace if no parent
    const content = nodes
      .map((node) => {
  ...
  public getElementsByTagName(tagName: string): Array<HTMLElement> {
    ...
    let currentNodeReference = this as Node | null;
    ...
    while (index !== undefined && currentNodeReference) {
  ...
  /** Prepend nodes or strings to this node's children. Strings containing HTML will be parsed. */
  ...
  public before(...insertable: NodeInsertable[]) {
    ...
    const nodes = resolveInsertable(insertable, (this.parentNode as HTMLElement)._parseOptions);
...
export function base_parse(data: string, options = {} as Partial<Options>) {
  const voidTag = new VoidTag(
    options?.voidTag?.closingSlash,
    options?.voidTag?.tags
  );
  ...
  let match: RegExpExecArray | null;
  // Wrap content in frame flag to handle edge cases with malformed HTML
  ...
  while ((match = kMarkupPattern.exec(data))) {
    ...
    // Skip frameflag node (internal wrapper)
    ...
    if (leadingSlash || closingSlash || voidTag.isVoidElement(tagName)) {
      while (true) {
        ...
        if (currentParent.rawTagName === tagName) {
          ...
          const parentTagName = currentParent.tagName as
            | 'LI'
            | 'A'
            | 'B'
            | 'I'
      ...
      if (last.parentNode === oneBefore && last.tagName === oneBefore.tagName) {
        ...
        if (options.parseNoneClosedTags !== true) {
          ...
          last.childNodes.forEach((child) => {
            if (oneBefore.parentNode) oneBefore.parentNode.appendChild(child);
          });
          stack.pop();
        }
      } else {
 ...
 *
 * If a string contains HTML tags (starts with '<'), it will be parsed as HTML.
 * Otherwise, it will be treated as a text node.
 */
function resolveInsertable(insertable: NodeInsertable[], options?: Partial<Options>): Node[] {
  const result: Node[] = [];
  for (const val of insertable) {
    if (typeof val === 'string') {
      // Check if string looks like HTML (contains < followed by letter/!)
      if (/<[a-zA-Z!]/.test(val)) {
        // Parse as HTML and add all child nodes
        // base_parse returns a stack, get the root element (first item)
