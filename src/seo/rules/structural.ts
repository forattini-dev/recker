...
export const structuralRules: SeoRule[] = [
  {
    ...
    check: (ctx) => {
      if (ctx.h1Count === undefined) return null;
      if (ctx.h1Count === 0) {
        return createResult(
      ...
      if (ctx.h1Count > 1) {
        return createResult(
          ...
          { value: ctx.h1Count, recommendation: 'Use only one H1 per page' }
        );
      }
      return createResult(
        { id: 'h1-exists', name: 'H1 Tag', category: 'headings', severity: 'error' },
  ...
  },
  {
    ...
    check: (ctx) => {
      if (ctx.h2Count === undefined) return null;
