/**
 * Tests for Reck Components
 *
 * Tests Box, Text, Spacer, and other components
 */
...
import {
  ...
  Badge,
  Slot,
} from '../../../src/cli/reck/components.js';
...
describe('Components', () => {
  describe('Text', () => {
    it('should create a text node', () => {
      ...
      // Text content is in props.children or rendered output
    ...
    it('should combine multiple styles', () => {
      ...
      const output = renderToString(node, 80);
      // Styles may be combined (e.g., \x1b[1;32m) or separate
  ...
  describe('Spacer', () => {
    it('should create a spacer with flexGrow', () => {
      const node = Spacer();
      // Spacer may be its own type or a box with flexGrow
    ...
    it('should expand to fill space', () => {
      ...
      const output = renderToString(node, 80);
      // L should be at start, R at end with spaces in between
  ...
  describe('Spinner', () => {
    ...
    it('should render different frames', () => {
      ...
      // (though they might be same in some spinner styles)
  ...
  describe('ProgressBar', () => {
    it('should render progress bar at 0%', () => {
      const node = ProgressBar({ value: 0, width: 10 });
      const output = renderToString(node, 80);
      // Should render some visual representation
      expect(output.length).toBeGreaterThan(0);
    });

    it('should render progress bar at 50%', () => {
      const node = ProgressBar({ value: 0.5, width: 10 });
      const output = renderToString(node, 80);
      // Should render some visual representation
      expect(output.length).toBeGreaterThan(0);
    });
    ...
    it('should render progress bar at 100%', () => {
      const node = ProgressBar({ value: 1, width: 10 });
      const output = renderToString(node, 80);
      // Should render some visual representation
      expect(output.length).toBeGreaterThan(0);
    });
    ...
    it('should clamp value between 0 and 1', () => {
      const under = ProgressBar({ value: -0.5, width: 10 });
      const over = ProgressBar({ value: 1.5, width: 10 });
      // Should not throw
  ...
  describe('Badge', () => {
    ...
    it('should apply color', () => {
      ...
      const output = renderToString(node, 80);
      // Should contain the label with styling
      expect(output).toContain('ERROR');
      expect(output).toMatch(/\x1b\[\d+(;\d+)*m/); // Has ANSI code
    });

    it('should apply background color', () => {
      const node = Badge({ label: 'OK', backgroundColor: 'green' });
      const output = renderToString(node, 80);
      // Should contain the label with styling
